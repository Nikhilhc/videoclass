...
def ResNet50(input_shape, num_classes=6):
...
def train_network():
    ...
    model = ResNet50((160, 120, 3), num_classes=num_classes)
    ...
    train_generator = train_datagen.flow_from_directory(DATASET_TRAIN_DIR, target_size=(160, 120),
                                                        batch_size=batch_size, class_mode="categorical")
    test_generator = test_datagen.flow_from_directory(DATASET_TEST_DIR, target_size=(160, 120), batch_size=batch_size,
...
MODEL_PATH = os.path.join(execution_path, "videoclass_weight_model.028-0.7625502008032129.h5")
JSON_PATH = os.path.join(execution_path, "model_class.json")
...
def run_inference():
    model = ResNet50(input_shape=(160, 120, 3), num_classes=6)
    model.load_weights(MODEL_PATH)

    picture = os.path.join(execution_path, "handwaving.jpg")
